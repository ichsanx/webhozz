		...
		<div class="row">
			...
			<br>
				<div class='row'>
					...
					include "admin/koneksi.php";
					$id=$_GET['id'];
					// query menggabungkan 2 table berdasarkan id_kategori
					$query = "SELECT * FROM artikel a JOIN kategori b ON a.kategori = b.id_kategori where a.id = '".$id."'";
					// $query	="SELECT * FROM artikel,kategori WHERE artikel.kategori=kategori.id_kategori AND artikel.id = '".$id."'";
					$tampil	=mysqli_query($conn, $query);
					while($data=mysqli_fetch_array($tampil)){
						$id 			=$data['id'];
						$judul			=$data['judul'];
